...
const App = () => {
    const [gameState, setGameState] = useState<'home' | 'quiz' | 'complete'>('home');
    ...
    if (gameState === 'home') {
        return (
            <div className="quiz-container home-screen">
                <h1>歡迎參加我們的糖尿病控糖知識大挑戰！</h1>
                <p>透過10個簡單問題，測試您對血糖控制的了解，學習如何更好地管理糖尿病。</p>
                <p>準備好了嗎？</p>
                <button className="start-btn" onClick={handleStartGame}>
                    開始遊戲！
                </button>
                 <p className="designer-credit">遊戲設計：彰化基督教醫院 內分泌暨新陳代謝科 張桐瑋管理師</p>
            </div>
        );
    }
    ...
    return (
        <div className="quiz-container">
            <h1>糖尿病控糖知識大挑戰</h1>
            <div className="level-indicator">關卡 {currentLevel + 1} / {quizData.length}</div>
            ...
            <div className="feedback-container">
                {feedback && (
                    <>
                        <p className={`feedback-message ${feedback === '答對了！你好棒！' ? 'correct-text' : 'incorrect-text'}`}>
                            {feedback}
                        </p>
                        {feedback === '答對了！你好棒！' && <p className="explanation-text">{currentQuestion.explanation}</p>}
                    </>
                )}
            </div>
            <p className="designer-credit">遊戲設計：彰化基督教醫院 內分泌暨新陳代謝科 張桐瑋管理師</p>
